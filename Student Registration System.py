...
def showimage():
    ...
    filename=filedialog.askopenfilename(initialdir=os.getcwd(),
                                        title="Select an Image", filetypes=(("JPG File","*.jpg"),
                                                                             ("PNG File","*.png"),
                                                                             ("All File","*.txt")))
...
#-----------------------------------------------Registration No.
    # we have to enter registration no each and every time
...
def Save():
    ...
    if N1=="" or C1 =="Select Class" or D2=="" or Re1=="" or S1=="" or fathername=="" or mothername=="" or F1=="" or M1=="":
        messagebox.showerror("error","Few Sections are missing!")
    else:
        file = openpyxl.load_workbook('Student_data.xlsx')
        sheet = file.active
        ...
        Clear()  # clear entered section and image
...
def search():
    ...
    Clear() # to clear all the data already availabe n the entry box and other
    ...
    #  reg no position showinf like a2 a3 a4   but registration number hust showind number after a2 like 2,3 3,4
...
def Update():
    ...
    #sheet.cell(column=1, row=int(reg_number),value=R1) noone can update registration number it will remain same
...
Label(root,text = "Email: Amorte@gmail.com", width= 10, height=3, bg="#7AC5CD", anchor='e').pack(side=TOP, fill = X)
Label(root,text = "STUDENT REGISTRATION", width= 10, height=2, bg="#5F9EA0", fg='#fff', font='arial 20 bold').pack(side=TOP, fill = X)
...
Srch=Button(root,text ="Search",compound = LEFT , image = imageicon3,width =123, bg = '#7AC5CD', font='arial 13 bold', command = search)
Srch.place(x = 1060, y=66)


imageicon4 = PhotoImage(file = "Images/Layer 4.png")
Update_button = Button(root, image = imageicon4, bg = "#E0EEEE", command = Update)
